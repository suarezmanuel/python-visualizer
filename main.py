...
from iterators import TreeIterator, TreeAbstractIterator, TreeLeafIterator
from dataclasses import dataclass, field
import os

MAX_TREE_DEPTH = 2

def _tree(depth, lines_arr, paddings, folder_path):
    ...
    for i, f in enumerate(files):
        ...
        if os.path.isdir(f_path):
            does_parent_have_next=(i < (len(files)-1))
            parent_str_len = 8+len(f)
            _tree(depth=depth+1, lines_arr=lines_arr+[does_parent_have_next], paddings=paddings+[parent_str_len], folder_path=f_path)

def tree(folder_path):
    if folder_path == "":
        return
    ...
    print(root)
    titer = TreeAbstractIterator(folder_path, before=before, on_node=on_node, on_leaf=on_leaf, state=CustomState(lines_arr=[False], paddings=[len(root)]))
    for _ in titer:
        pass
...
def get_leaves(folder_path):
    titer = TreeLeafIterator(folder_path)
    result = []
    for i in titer:
        result += [i]
    return result
...
tree("/Users/manuel/Code/minecraft")

# print(get_leaves("/Users/manuel/Code/minecraft"))
